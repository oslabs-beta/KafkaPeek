...
const BytesInPer = ({metrics, metric, bytesInFunc, bytesInPerInterval}) => { 
  const intervals = [
  ...
  return (
    ...
    <h2 style={{marginBottom:'15px',fontSize:'3.6rem',lineHeight:'1.25',letterSpacing:'-.1rem',marginTop: '100px'}}>BytesInPerSec</h2>
    ...
    The following options are thresholds that you can pick from for your BytesInPerSec Metric. 
      ...
      <div className="subContainer">
        <div className="innerBoxContainer" style={{float:'left',boxSizing:'border-box'}}>
          <Select width='50px' menuColor='red' onChange={(d)=> bytesInFunc}options={intervals} value={bytesInPerInterval} />
        </div>
    </div>
  </div>
  );
